...
public class UpdateV2Test {
    ...
    public static String url = "http://192.168.21.211:31020/data/api/v2/risk/rule/updateV2";
    ...
    public void test() throws Exception{
        ...
        rule.put("ruleCode", "14000001");
        rule.put("ruleType", "监管风险");
        rule.put("ruleDimension", "严重违法");
        rule.put("riskLevelRule", "企业被列入严重违法");
        rule.put("ruleLevel", "中风险");
        rule.put("ordinalPosition", "11");
        rule.put("enabled", 1);
        rules.add(rule);

        Map<String, Object> paramMap = new LinkedHashMap<>();
        paramMap.put("rules", rules);
        System.out.println(">>>>>>>>请求数据: " + JSONUtil.toJsonPrettyStr(paramMap));

        // 使用公钥加密请求数据
        String encryptData = encrypt(JSONUtil.toJsonStr(paramMap), publicKeyBase64);
        ...
        System.out.println("请求报文: " + JSONUtil.toJsonPrettyStr(paramMap));
